import type { GradingClassroomOption, GradingExamOption, GradingSubjectOption } from '../types/grading.types'

type GradingFilterBarProps = {
  subjects: GradingSubjectOption[]
  classrooms: GradingClassroomOption[]
  exams: GradingExamOption[]
  subjectId: number | ''
  classroomId: number | ''
  examId: number | ''
...
export function GradingFilterBar({
  subjects,
  classrooms,
  exams,
  subjectId,
  ...
  onClassroomChange,
  onExamChange,
}: GradingFilterBarProps) {
  return (
    <div className="grid grid-cols-1 gap-3 rounded-2xl border border-slate-200 bg-white p-4 lg:grid-cols-3">
      <div className="space-y-1.5">
        <label htmlFor="grading-subject" className="text-xs font-medium uppercase tracking-wider text-slate-500">
          Môn học
        </label>
        <select
          id="grading-subject"
      ...
      <div className="space-y-1.5">
        <label htmlFor="grading-classroom" className="text-xs font-medium uppercase tracking-wider text-slate-500">
          Lớp học
        </label>
        <select
          id="grading-classroom"
          value={classroomId}
          disabled={!subjectId}
      ...
      <div className="space-y-1.5">
        <label htmlFor="grading-exam" className="text-xs font-medium uppercase tracking-wider text-slate-500">
          Đề thi
        </label>
        <select
          id="grading-exam"
